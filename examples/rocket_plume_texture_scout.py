...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render a plume texture scout grid, or promote one picked tile.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.pick:
        ...
        render_selected_from_sweep(
            ...
            root=ROOT,
            config=config,
            title="Rocket Vacuum Plume Texture Selected Render",
            notes=[
                "Promoted after inspecting the plume texture contact sheet.",
                "Use this heavier render to judge whether density texture reads billowy instead of noisy.",
            ],
        )
        return
    ...
    render_sweep(
        ...
        promotion_command="/Applications/Blender.app/Contents/MacOS/Blender --background --python examples/rocket_plume_texture_scout.py -- --pick {pick}",
        square=True,
    )
