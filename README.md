...
The reference manifest verifies that local PDFs and SpaceX-derived visual references are present and unchanged before an agent downloads or regenerates media. The PDF helper writes backend status, any extracted text or thumbnails, and a `notes.md` stub under `runs/pdf_triage/`.
